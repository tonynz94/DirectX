...
//특수 종이를 2개 만들어서, 하나는 현재 화면에 그려주고, 하나는 외주 맡기고
//Double buffering
...
private:
	...
	ComPtr<ID3D12Resource>				    _rtvBuffer[SWAP_CHAIN_BUFFER_COUNT]; //특수종이(Buffer)
	ComPtr<ID3D12DescriptorHeap>			_rtvHeap;
	D3D12_CPU_DESCRIPTOR_HANDLE		_rtvHandle[SWAP_CHAIN_BUFFER_COUNT];
	
	
	uint32							_backBufferIndex = 0; //현재 뒷작업이 누구인가 0 1 0 1 0 1 왔다갔다 함.
};
